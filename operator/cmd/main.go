...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		Scheme: scheme,
		// ClaimBinder is the only pod reader, and only for unbound claims.
		// Caching would hold every pod on the node (virt-launcher and CDI
		// importers on a KubeVirt host) to serve a rare, bursty lookup.
		...
		LeaderElectionID:       "f733c7a3.stornas.io",
	...
	if err := (&controller.LocalUserReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "LocalUser")
		os.Exit(1)
	}
	if err := (&controller.ClaimBinder{
		Client:  mgr.GetClient(),
		Linstor: placer,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "ClaimBinder")
		os.Exit(1)
	}
