...
# The UI restore declares a host consumer, so the binder pins it to a
# diskful node; the replicated volume attaches on node1 either way.
# The podless restore assert is boot-test's row.
kc apply -f - <<'EOF'
apiVersion: v1
kind: Pod
metadata:
